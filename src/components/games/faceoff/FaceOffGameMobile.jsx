...
const FaceOffGameMobile = ({ config, players, onGameEnd, onBackToSetup, onExitToPortal, selectedClass, onGivePoints }) => {
  const gameRef = useRef(null);
  const soundManagerRef = useRef(null);
  
  const [currentRound, setCurrentRound] = useState(0);
  const [totalRounds, setTotalRounds] = useState(config?.rounds || 5);
  const [gameMode, setGameMode] = useState(config?.mode || 'words');
  const [wordImagePairs] = useState(config?.wordImagePairs || []);
  
  const [currentPair, setCurrentPair] = useState(null);
  const [displayItems, setDisplayItems] = useState({ top: [], bottom: [] });
  ...
  const loadRound = useCallback(async () => {
    ...
    const allUniqueWordsUsed = usedPairs.length >= wordImagePairs.length;
    ...
    let availablePairs = wordImagePairs.filter((_, index) => !usedPairs.includes(index));

    if (availablePairs.length === 0 && !isCustomRounds) {
      endGame();
      return;
    } else if (availablePairs.length === 0 && isCustomRounds) {
      availablePairs = [...wordImagePairs];
    }

    const pairIndex = Math.floor(Math.random() * availablePairs.length);
    const pair = availablePairs[pairIndex];
    const actualIndex = wordImagePairs.indexOf(pair);

    if (!(isCustomRounds && usedPairs.length >= wordImagePairs.length)) {
      setUsedPairs(prev => [...prev, actualIndex]);
    }
    ...
    if (gameMode === 'words') {
      await loadWordsModeItems(pair);
    } else {
      await loadPicturesModeItems(pair);
    }
  }, [currentRound, totalRounds, wordImagePairs, usedPairs, gameMode, endGame]);

  const loadWordsModeItems = async (pair) => {
    const availableDistractors = wordImagePairs.filter(p => p.image !== pair.image);
    // Deduplicate images to avoid duplicates in options
    const uniqueImages = [...new Set(availableDistractors.map(p => p.image))];
    let images = [pair.image];

    if (uniqueImages.length >= 5) {
      const shuffled = [...uniqueImages].sort(() => Math.random() - 0.5);
      images = [...images, ...shuffled.slice(0, 5)];
    } else if (uniqueImages.length > 0) {
      const distractors = uniqueImages;
      const needed = 6 - images.length;
      for (let i = 0; i < needed; i++) {
        images.push(distractors[Math.floor(Math.random() * distractors.length)]);
      }
    } else {
      for (let i = 0; i < 5; i++) images.push(pair.image);
    }

    const shuffledImages = images.sort(() => Math.random() - 0.5);
    const itemsWithCorrect = shuffledImages.map(img => ({
      image: img,
      isCorrect: img === pair.image
    }));

    setDisplayItems({ top: [...itemsWithCorrect], bottom: [...itemsWithCorrect] });
  };

  const loadPicturesModeItems = async (pair) => {
    const availableDistractors = wordImagePairs.filter(p => p.word !== pair.word);
    // Deduplicate words to avoid duplicates in options
    const uniqueWords = [...new Set(availableDistractors.map(p => p.word))];
    let words = [pair.word];

    if (uniqueWords.length >= 5) {
      const shuffled = [...uniqueWords].sort(() => Math.random() - 0.5);
      words = [...words, ...shuffled.slice(0, 5)];
    } else if (uniqueWords.length > 0) {
      const distractors = uniqueWords;
      const needed = 6 - words.length;
      for (let i = 0; i < needed; i++) {
        words.push(distractors[Math.floor(Math.random() * distractors.length)]);
      }
    } else {
      for (let i = 0; i < 5; i++) words.push(pair.word);
    }

    const shuffledWords = words.sort(() => Math.random() - 0.5);
    const itemsWithCorrect = shuffledWords.map(word => ({
      word: word,
      isCorrect: word === pair.word
    }));

    setDisplayItems({ top: [...itemsWithCorrect], bottom: [...itemsWithCorrect] });
  };

  const handleItemClick = async (playerIndex, itemIndex, isCorrect) => {
    if (roundComplete || clickedItems.has(`${playerIndex}-${itemIndex}`)) return;

    const clickedKey = `${playerIndex}-${itemIndex}`;
  ...
  useEffect(() => {
    if (!loading && currentRound === 0 && !currentPair) {
      loadRound();
    }
  }, [loading, currentRound, currentPair, loadRound]);
  ...
  if (loading) {
    return (
      <div style={styles.loadingContainer}>
        ...
        <div style={styles.loadingText}>Loading FaceOff...</div>
      </div>
    );
  }
    ...
    <div style={styles.container}>
      ...
      <div style={{ ...styles.playerSection, transform: 'rotate(180deg)' }}>
        <div style={styles.itemsGrid}>
          {displayItems.bottom.map((item, idx) => {
            ...
            return (
              ...
              >
                {gameMode === 'words' ? (
                  ...
                  <div style={styles.itemWord}>{item.word}</div>
                )}
              </div>
            );
          })}
      ...
      {/* Central Dual Question Display */}
      <div style={styles.dualQuestionContainer}>
        <div style={{ ...styles.questionBox, transform: 'rotate(180deg)' }}>
          {gameMode === 'words' ? (
            <div style={styles.questionText}>{currentPair?.word || ''}</div>
          ) : (
            <img src={currentPair?.image || ''} alt="Question" style={styles.questionImage} />
          )}
        </div>
...
export default FaceOffGameMobile;